...
export default function SingleMovieModalEdit(props) {
  ...
  const {
    ...
    defaultValues: {
      ...
      awards:
        awards != undefined && awards?.text?.length !== 0 ? awards.text : "N/A",
  ...
  async function updateMovie(userID, data) {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/movies/${userID}`,
      {
        method: `PUT`,
        body: JSON.stringify({
          data: data,
  ...
  return (
    <Form style={{ textAlign: "left" }} onSubmit={handleSubmit(submitForm)}>
      <Form.Group className="mb-3" controlId="formBasicEmail">
        <Form.Label>Title</Form.Label>
        <Form.Control {...register("title")} className="mb-1" type="text" />
        <Form.Label>Director</Form.Label>
        <Form.Control {...register("directors")} className="mb-1" type="text" />
        <Form.Label>Cast</Form.Label>
        <Form.Control {...register("cast")} className="mb-1" type="text" />
        <Form.Label>Genre</Form.Label>
        <Form.Control {...register("genre")} className="mb-1" type="text" />
        <Form.Label>Motion Picture Rating (MPAA):</Form.Label>
        <Form.Control {...register("rated")} className="mb-1" type="text" />
        <Form.Label>IMDb</Form.Label>
        <Form.Control {...register("imdb")} className="mb-1" type="text" />
        <Form.Label>Awards</Form.Label>
        <Form.Control {...register("awards")} className="mb-1" type="text" />
